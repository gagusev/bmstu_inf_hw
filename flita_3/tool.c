...
int searchGraph(Graph *graph) 
{
    Vertex *ptr = graph->head[0];
    graph->head[0]->visited = 1;
    ...
    while(ptr != NULL) 
    {
        graph->head[ptr->dest]->visited = 1;
        queue[queue_size] = graph->head[ptr->dest];
        queue_size++;
        ptr = ptr->next;
    }
    ...
    while(queue_size != 0)
    {
        Vertex *ptr = queue[queue_size-1];
        queue[queue_size-1] = NULL;
        queue_size--;
        while(ptr != NULL) 
        {
            if (graph->head[ptr->dest]->visited == 0) 
            {
                graph->head[ptr->dest]->visited = 1;
                queue[queue_size] = graph->head[ptr->dest];
                queue_size++;
...
void saveGraph(Graph *graph, char graph_name[], int is_disconnected)
{
    ...
    {
        Vertex *ptr = graph->head[i];
        if (ptr->visited == 1 && is_disconnected) {
...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 3 && (!strcmp(argv[1], "LOAD") || !strcmp(argv[1], "-l") || !strcmp(argv[1], "-L")))
    {
        ...
        int is_disconnected = searchGraph(graph);
        printGraph(graph);
        saveGraph(graph, graph_name, is_disconnected);
